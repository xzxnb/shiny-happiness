...
def main(proc_id, filenames, output_filenames):
    syn = Synthesisability()
    while True:
        for filename, output_filename in zip(filenames, output_filenames):
            print(f"Calculating for {filename}, {output_filename} <<<<-----")
            selected_mol = None
            ...
            print(
                "====Working for sample {}/{}====".format(selected_mol[0], num_samples)
            )
            try:
...
if __name__ == "__main__":
    ...
    main(args.proc_id, args.filenames, args.output_filenames)
