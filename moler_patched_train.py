    ...
    with dataset.get_context_managed_tf_dataset(
        ...
        with dataset.get_context_managed_tf_dataset(
            ...
            for epoch in range(1, max_epochs + 1):
                ...
                print(
                    os.system(
                        f"molecule_generation sample {save_dir} 512 >> {save_dir}/generated_samples.in-training.txt"
                    )
                )

                # with VaeWrapper(save_dir) as model_wrapper:
                #     with open(
                #         f"{save_dir}/{run_id}_generated_samples.in-training.txt", "a"
                #     ) as f:
                #         for sample in model_wrapper.sample(512):
                #             f.write(f"{sample}\n")
