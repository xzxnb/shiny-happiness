...
from dgd.analysis.rdkit_functions import build_molecule, mol2smiles
...
class LiftedDenoisingDiffusion(pl.LightningModule):
    def __init__(
        ...
        diffusion_utils.check_issues_norm_values(
            self.gamma, self.norm_values[1], self.norm_values[2]
        )
    ...
    def training_step(self, data, i):
        ...
        normalized_data = utils.normalize(
            X, E, data.y, self.norm_values, self.norm_biases, node_mask
        )
        noisy_data = self.apply_noise(
            normalized_data.X, normalized_data.E, normalized_data.y, node_mask
        )
    ...
    def validation_epoch_end(self, outs) -> int:
        metrics = [
            self.val_nll.compute(),
            self.val_X_mse.compute(),
            self.val_E_mse.compute(),
            self.val_y_mse.compute(),
        ...
        print(
            f"Epoch {self.current_epoch}: Val NLL {metrics[0] :.2f} -- Val Atom type MSE {metrics[1] :.2f} -- ",
    ...
    def test_epoch_end(self, outs) -> None:
        ...
        metrics = [
            ...
            self.test_X_mse.compute(),
            self.test_E_mse.compute(),
            self.test_y_mse.compute(),
        ...
        print(
            f"Epoch {self.current_epoch}: Test NLL {metrics[0] :.2f} -- Test Atom type MSE {metrics[1] :.2f} -- ",
    ...
    def reconstruction_logp(
        self, data, data_0, gamma_0, eps, pred_0, node_mask, epsilon=1e-10, test=False
    ):
        """Reconstruction loss.
        output size: (1).
        """
        ...
        unnormalized_data = utils.unnormalize(
            X, E, y, self.norm_values, self.norm_biases, node_mask, collapse=False
        )
        unnormalized_0 = utils.unnormalize(
            X_0, E_0, y_0, self.norm_values, self.norm_biases, node_mask, collapse=False
        )
        ...
        logps = utils.PlaceHolder(
            X=log_probabilities_X * unnormalized_data.X,
    ...
    def apply_noise(self, X, E, y, node_mask):
        ...
        gamma_s = diffusion_utils.inflate_batch_array(
            self.gamma(s_normalized), X.size()
        )  # (bs, 1, 1),
        gamma_t = diffusion_utils.inflate_batch_array(
            self.gamma(t_normalized), X.size()
        )  # (bs, 1, 1)

        # Compute alpha_t and sigma_t from gamma, with correct size for X, E and z
        alpha_t = diffusion_utils.alpha(
            gamma_t, X.size()
        )  # (bs, 1, ..., 1), same n_dims than X
        sigma_t = diffusion_utils.sigma(
            gamma_t, X.size()
        )  # (bs, 1, ..., 1), same n_dims than X

        # Sample zt ~ Normal(alpha_t x, sigma_t)
        eps = diffusion_utils.sample_feature_noise(
            X.size(), E.size(), y.size(), node_mask
        ).type_as(X)
    ...
    def compute_val_loss(self, pred, noisy_data, X, E, y, node_mask, test=False):
        ...
        delta_log_pE = (
            -self.Edim_output * 0.5 * N * (N - 1) * np.log(self.norm_values[1])
        )
        kl_prior = (
            kl_prior_without_prefactor - delta_log_px - delta_log_py - delta_log_pE
        )
        ...
        if test:
            diffusion_error = (
                self.test_X_mse(weighted_predX_diffusion, weighted_epsX_diffusion)
            ...
            diffusion_error = (
                self.val_X_mse(weighted_predX_diffusion, weighted_epsX_diffusion)
        ...
        gamma_0 = diffusion_utils.inflate_batch_array(
            self.gamma(t_zeros), X_t.size()
        )  # bs, 1, 1
        ...
        eps0 = diffusion_utils.sample_feature_noise(
            X_t.size(), E_t.size(), y_t.size(), node_mask
        ).type_as(X_t)
        ...
        loss_term_0 = -self.reconstruction_logp(
            data={"X": X, "E": E, "y": y},
        ...
        nll = (
            self.test_nll(nlls) if test else self.val_nll(nlls)
    ...
    def sample_batch(
        ...
        z_T = diffusion_utils.sample_feature_noise(
            X_size=(batch_size, n_nodes_max, self.Xdim_output),
    ...
    def sample_discrete_graph_given_z0(self, X_0, E_0, y_0, node_mask):
        ...
        pred_X = 1.0 / alpha_0 * (X_0 - sigma_0 * eps0.X)
        pred_E = 1.0 / alpha_0.unsqueeze(1) * (E_0 - sigma_0.unsqueeze(1) * eps0.E)
        pred_y = 1.0 / alpha_0.squeeze(1) * (y_0 - sigma_0.squeeze(1) * eps0.y)
        assert (pred_E == torch.transpose(pred_E, 1, 2)).all()

        sampled = diffusion_utils.sample_normal(
            pred_X, pred_E, pred_y, sigma, node_mask
        ).type_as(pred_X)
        assert (sampled.E == torch.transpose(sampled.E, 1, 2)).all()

        sampled = utils.unnormalize(
            sampled.X,
            sampled.E,
            sampled.y,
            self.norm_values,
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask):
        ...
        (
            ...
            sigma_t_given_s,
            alpha_t_given_s,
        ) = diffusion_utils.sigma_and_alpha_t_given_s(gamma_t, gamma_s, X_t.size())
        ...
        mu_X = (
            X_t / alpha_t_given_s
            - (sigma2_t_given_s / (alpha_t_given_s * sigma_t)) * eps.X
        )
        mu_E = (
            E_t / alpha_t_given_s.unsqueeze(1)
            - (sigma2_t_given_s / (alpha_t_given_s * sigma_t)).unsqueeze(1) * eps.E
        )
        mu_y = (
            y_t / alpha_t_given_s.squeeze(1)
            - (sigma2_t_given_s / (alpha_t_given_s * sigma_t)).squeeze(1) * eps.y
        )
        ...
        z_s = diffusion_utils.sample_normal(mu_X, mu_E, mu_y, sigma, node_mask).type_as(
            mu_X
        )
