...
def sample_discrete_features(probX, probE, node_mask):
    ...
    diag_mask = torch.zeros(probE.size(0), probE.size(1), probE.size(2)) + torch.eye(
    ...
    probE = probE.reshape(
        probE.size(0) * probE.size(1) * probE.size(2), -1
    )  # (bs * n * n, de_out)

    # Sample E
    E_t = probE.multinomial(1).reshape(
        node_mask.size(0), node_mask.size(1), node_mask.size(1)
    )  # (bs, n, n)
