...
def generate_mask(
    ...
    for neighbor in range(real_n_vertices):
        if (
            neighbor != node_in_focus
            and color[neighbor] < 2
            and not check_adjacent_sparse(adj_mat, node_in_focus, neighbor)[0]
        ):
...
def generate_label(
    ...
    edge_label = []
    for neighbor in range(real_n_vertices):
        adjacent, edge_type = check_adjacent_sparse(
            ground_truth_graph, node_in_focus, neighbor
        )
        ...
        if not params["label_one_hot"] and adjacent and not incre_adjacent:
            ...
            params["label_one_hot"]
            and adjacent
            and not incre_adjacent
            and neighbor == real_neighbor
        ):
            edge_type_label.append((node_in_focus, neighbor, edge_type))
            edge_label.append((node_in_focus, neighbor))
...
def update_one_step(
    ...
    edge_type_mask, edge_mask = generate_mask(
        valences,
        up_to_date_adj_mat,
        color,
        real_n_vertices,
    ...
    if not local_stop_label:
        ...
        edge_type_label, edge_label = generate_label(
            graph, up_to_date_adj_mat, node_in_focus, neighbor, real_n_vertices, params
        )
    ...
    distances = [
        (start, node, params["truncate_distance"])
        if d > params["truncate_distance"]
        else (start, node, d)
        for start, node, d in distances
    ]
...
# given a molecule in input, it saves every steps to do in order to reconstruct it following the input params
def construct_incremental_graph(
    dataset, edges, max_n_vertices, real_n_vertices, node_symbol, params, initial_idx=0
):
    # avoid calculating this if it is just for generating new molecules for speeding up
    if params["generation"] not in [
        0,
        3,
    ]:  # in this way is returns all [] when the model is set to generation and reconstruction
        return [], [], [], [], [], [], [], [], []
    ...
    # Construct a graph object using the edges. Every edge is associated to (destinazione, tipo edge)
    ...
    while len(queue) > 0:
        ...
        for neighbor, edge_type in current_adj_list:
            ...
            if color[neighbor] < 2:
                ...
                new_mol.AddBond(
                    int(node_in_focus), int(neighbor), number_to_bond[edge_type]
                )
