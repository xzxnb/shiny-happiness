...
class SMILESDataset(Dataset):
    def __init__(self, smiles_file, percentage, vocab):
        ...
        if self.vocab.name == "selfies":
            self.data = [
                sf.encoder(x)
                for x in self.read_smiles_file(smiles_file)
                if sf.encoder(x) is not None
            ]
        else:
        ...
        self.data = [
            x
            for x in self.data
            if none_on_exp(lambda: self.vocab.tokenize_smiles(x)) is not None
        ]
        print("total number of SMILES loaded LEFT: ", len(self.data))
