...
def main(  # classify_two_mlns
        folder: str,
        val_folder: Optional[str] = "wfomi_data/json/color1_10k_mln/domain10",
        seed: int = 0,
        gpu: int = 0,
        dry: bool = False,
        user: str = "color1_10k_skip_v10_sum",
):
    data_family = folder.split("json/")[1]
    data_family = folder.split("json/")[1].split("/")[0]
...
class GNN(pl.LightningModule):
    ...
    def process_subgraph(self, node_features, adj_matrix):
        ...
        # batch_size, n_nodes, n_feat = node_features.shape
        # zero_row = torch.zeros(
        #     batch_size, 1, n_feat,
        #     device=node_features.device,  # 保持设备一致
        #     dtype=node_features.dtype     # 保持数据类型一致
        # )
        # zero_row[:, 0, 0] = 1
        # # 在第二维（dim=1）拼接，得到[batch_size, n_nodes+1, n_feat]
        # node_features = torch.cat([node_features, zero_row], dim=1)
        ...
        # )
        # # 在第二维（dim=1）拼接，得到[batch_size, n_nodes+1, n_nodes]
        # adj_matrix = torch.cat([adj_matrix, ones_row], dim=1)

        # ones_col = torch.ones(
        # batch_size, n_nodes + 1, 1,  # 第二维是n_nodes+1（因为已新增一行）
        # device=adj_matrix.device,
        # dtype=adj_matrix.dtype
        # )
        # # 拼接后形状变为[batch_size, n_nodes+1, n_nodes+1]
        # adj_matrix = torch.cat([adj_matrix, ones_col], dim=2)
    ...
    def validation_epoch_end(self, outputs):
        ...
        precisions = [x["val_precision"] for x in outputs]
        recalls = [x["val_recall"] for x in outputs]
        batch_sizes = [x["batch_size"] for x in outputs]  # 权重：每个批次的样本数
