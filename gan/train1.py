...
def main(  # classify_two_mlns
    ...
    gpu: int = 0,
    dry: bool = False,
    user: str = "SAGEConv_readout_addnode",
):
    data_family = folder.split("json/")[1].split("/")[0].split("_")[0][:-1]
    domain_size = int(folder.split("domain")[1].split("/")[0]) +1
...
def train(
    ...
    gnn = GNN(
        n_node_classes=data_family_name_to_n_node_classes[data_family]+1,
...
""",
        ...
        logger_mlflow.log_hyperparams(
            {
                ...
                "auto_train_ratio": auto_train_ratio,
                "seed": seed,
                "model": "DenseSAGEConv_add_node",
                "depth": 8,
                "isolated_nodes": "NO_self-loop",
                "EarlyStopping": 20,
                "auto_train_ratio": 0.7,
            }
        )
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[EarlyStopping(monitor="val_accuracy", mode="max", patience=20)],
    )
    trainer.fit(
        model=gnn,
...
class GNN(pl.LightningModule):
    ...
    def process_subgraph(self, node_features, adj_matrix):
        ...
        for i, layer in enumerate(self.hidden_layers):
            ...
            x = torch.nn.functional.tanh(x)
            graph_embedding = torch.mean(x, dim=1, keepdim=True)
            graph_embedding = self.readout[i](graph_embedding)
            # 加法融合：广播图级表示到所有节点并相加
            batch_size, node_num, feat_dim = x.shape
            repeated_graph_emb = graph_embedding.repeat(1, node_num, 1)  # [batch, node_num, feat_dim]
            x = x + repeated_graph_emb
